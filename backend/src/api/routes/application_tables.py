"""Application database table query API routes.
...
from api.application_tables import get_table, list_tables, list_categories, TableDefinition
from api.models.database import (
    MetadataTableInfo as ApplicationTableInfo,
    MetadataTableColumnInfo as ApplicationTableColumnInfo,
    TableCategoryInfo,
)
...
router = APIRouter(prefix="/api/application/tables", tags=["application-tables"])

# Cache for table list (avoids expensive pg_catalog queries)
_table_info_cache: dict[str, tuple[list[ApplicationTableInfo], float]] = {}
        ...
        try:
            ...
            if stats:
                ...
                if estimate < 50000:
                    ...
                    stale = True
                elif estimate > 0 and mod_since > estimate * 0.2:
                    stale = True
...
def _build_table_info(definition: TableDefinition) -> ApplicationTableInfo:
...
@router.get("", response_model=list[ApplicationTableInfo])
def list_application_tables():
    """List all available application database tables with column info (cached for 30s)."""
    cache_key = "application_tables"
    now = time.time()
...
@router.get("/categories", response_model=list[TableCategoryInfo])
def list_table_categories():
    """List all table categories for UI organization."""
    return [
        TableCategoryInfo(
            id=cat.id,
            label=cat.label,
            description=cat.description,
            table_names=list(cat.table_names),
        )
...
def query_application_table(table_name: str, payload: DataTablesRequest):
    """Query an application table with DataTables-style pagination, filtering, and sorting."""
    try:
    ...
    # Serialize values to handle datetime, enums, etc.
    data = [{k: _serialize_value(v) for k, v in row.items()} for row in result]
